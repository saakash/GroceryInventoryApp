...
//Function prototypes of milestone 5
...
//** The logic of this function is used from the course note **
int getInt(void) {
...
struct Item itemEntry(int sku) {
	...
	printf("        SKU: %d\n", item.sku);
	printf("       Name: ");
	scanf("%[^\n]s", item.name);    // ** the format %[^\n]s was used from internet (explained in comments.txt) **
...
// redundancy function() saves the repititive statements of switch cases 3, 4 and 5 on itemEntry function
void redundancy(const struct Item* item, char fileName[], int NoRecs) {
	int result = 0;
...
void deleteItem(struct Item item[],int *NoOfRecs) {
	int i,sku,yesNo,searchIndex, locateResult;
	printf("Please enter the SKU: ");
	sku = getInt();
	locateResult = locateItem(item, *NoOfRecs, sku, &searchIndex);
	if (locateResult == 0) {
		...
		if (yesNo == 1) {
			...
			loadItems(item, DATAFILE, NoOfRecs);
			printf("Deleted!\n");
		}
		else {
			printf("Aborted!\n");
